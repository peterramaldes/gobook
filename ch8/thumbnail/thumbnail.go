...
func makeThumbnails2(filenames []string) {
	for _, f := range filenames {
		go ImageFile(f) // NOTE: ignored errors
	}
}

// makeThumbnails3 parallel
func makeThumbnails3(filenames []string) {
	ch := make(chan struct{})
	for _, f := range filenames {
		go func(f string) {
			ImageFile(f) // NOTE: Ignored errors
			ch <- struct{}{}
		}(f)
	}
...
func makeThumbnails4(filenames []string) error {
	...
	for range filenames {
		if err := <-errors; err != nil {
			return err // NOTE: wrong, memory leak
		}
	}
	return nil
}
...
func makeThumbnails6(filenames <-chan string) int64 {
	...
	for f := range filenames {
		...
		go func(f string) {
			...
			info, _ := os.Stat(thumb) // it's ok ignore the error in this case
			sizes <- info.Size()
		}(f)
	}
